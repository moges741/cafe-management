...
export default function RecipeBuilderPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-foreground">Recipe Builder (Bill of Materials)</h1>
        ...
        <div className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-4 lg:col-span-1">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 bg-background border border-border rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-[#c29570]"
            />
          </div>

          <div className="divide-y divide-border max-h-[450px] overflow-y-auto space-y-1 pr-1">
            {isProductsLoading ? (
              <p className="text-center py-8 text-sm text-muted-foreground">Loading products...</p>
            ) : filteredProducts.length === 0 ? (
              <p className="text-center py-8 text-sm text-muted-foreground">No products found</p>
            ) : (
          ...
          {selectedProduct ? (
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-3 pb-4 border-b border-border">
                <ChefHat size={24} className="text-[#c29570]" />
                <div>
                  <h2 className="text-xl font-bold">{selectedProduct.name} Recipe</h2>
              ...
              <div className="space-y-3">
                ...
                {localIngredients.length === 0 ? (
                  <div className="p-8 text-center border border-dashed border-border rounded-xl text-muted-foreground text-sm flex flex-col items-center">
                    <Info size={24} className="mb-2 text-[#c29570] opacity-40" />
                  ...
                  <div className="divide-y divide-border border border-border rounded-xl bg-background overflow-hidden">
                    {localIngredients.map((item, idx) => {
                      ...
                      return (
                        <div key={item.materialId} className="flex items-center justify-between p-3.5 hover:bg-muted/10 transition-colors">
                          <div className="flex items-center gap-3">
                            <span className="text-xs text-muted-foreground font-mono">#{idx + 1}</span>
                            <div>
                              <div className="font-semibold text-sm">{material?.name ?? 'Unknown Material'}</div>
                              <div className="text-xs text-muted-foreground">Category: {material?.category}</div>
                            </div>
                          </div>

                          <div className="flex items-center gap-6">
                            <div className="font-bold text-sm text-[#c29570]">
                              {item.quantity} <span className="text-xs text-muted-foreground font-medium">{material?.unit}</span>
                            </div>
                            <button
              ...
              <div className="flex justify-end pt-4 border-t border-border">
                <Button
                  ...
                  className="bg-[#c29570] hover:bg-[#b08460] text-white rounded-xl flex items-center gap-2 font-semibold"
                >
                  <Save size={18} />
                  Save Recipe Config
                </Button>
            ...
            <div className="text-center py-24 text-muted-foreground flex flex-col items-center justify-center">
              <ChefHat size={48} className="mb-3 text-[#c29570] opacity-35" />
              <p className="text-lg font-medium text-foreground">Select a product from the list</p>
              <p className="text-sm">Configure raw materials composition for auto-consumption.</p>
            </div>
          )}
        </div>
      </div>
